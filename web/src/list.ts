...
export async function openListOverlay(modelId: string, decisionId: string, baseNames: string[], onSaved?: (newModelId: string) => void, opts?: { readOnly?: boolean; anchor?: Anchor }): Promise<void> {
  let lv: ListView | null = null
  try {
    lv = await getList(modelId, decisionId, opts?.anchor)
  } catch (e) {
    console.error(e)
    return
  }
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      const saved = await saveList(modelId, decisionId, { items: items.map((s) => s.trim()).filter((s) => s !== '') }, opts?.anchor)
