...
export type LaidNode = { id: string; type: string; name: string; dataType?: string; varName?: string; hasTable?: boolean; hasLiteral?: boolean; hasContext?: boolean; hasConditional?: boolean; hasLogic?: boolean; x: number; y: number; w: number; h: number }
  ...
  const pack = (): void => {
    for (const [r, ids] of rowIds) {
      ...
      for (const id of ids) {
        ...
        pos.set(id, { id, type: n.type, name: n.name, dataType: n.dataType, varName: n.varName, hasTable: n.hasTable, hasLiteral: n.hasLiteral, hasContext: n.hasContext, hasConditional: n.hasConditional, hasLogic: n.hasLogic, x, y, w: s.w, h: s.h })
        x += s.w + COL_GAP
      }
    }
  }
...
export function layout(graph: Graph): Laid {
  ...
  if (hasLayout) {
    for (const n of graph.nodes) {
      pos.set(n.id, { id: n.id, type: n.type, name: n.name, dataType: n.dataType, varName: n.varName, hasTable: n.hasTable, hasLiteral: n.hasLiteral, hasContext: n.hasContext, hasConditional: n.hasConditional, hasLogic: n.hasLogic, x: n.x ?? 0, y: n.y ?? 0, w: n.width ?? 0, h: n.height ?? 0 })
    }
  } else {
