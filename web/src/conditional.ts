...
export async function openConditionalOverlay(modelId: string, decisionId: string, baseNames: string[], onSaved?: (newModelId: string) => void, opts?: { readOnly?: boolean; anchor?: Anchor }): Promise<void> {
  let cv: ConditionalView | null = null
  try {
    cv = await getConditional(modelId, decisionId, opts?.anchor)
  } catch (e) {
    console.error(e)
    return
  }
