...
// queryClio is a clio stub that answers run-query with a fixed NDJSON body and
...
func reportServer(t *testing.T, q *queryClio) http.Handler {
	t.Helper()
	stub := q.start(t)
...
func TestQualityReportEndpointAggregates(t *testing.T) {
	q := &queryClio{body: strings.Join([]string{
...
func TestQualityReportEndpoint502OnClioError(t *testing.T) {
	q := &queryClio{status: http.StatusForbidden}
	h := reportServer(t, q)
	rec := do(t, h, "GET", "/v1/quality/report", "", nil)
