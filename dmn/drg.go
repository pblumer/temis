...
func (d *Definitions) Graph() Graph {
	...
	add := func(id, typ, name, dataType, varName string, hasTable, hasLiteral, hasContext, hasConditional, hasLogic bool) {
		if id == "" {
			return
		}
		known[id] = true
		n := GraphNode{ID: id, Type: typ, Name: name, DataType: dataType, VarName: varName, HasTable: hasTable, HasLiteral: hasLiteral, HasContext: hasContext, HasConditional: hasConditional, HasLogic: hasLogic}
	...
	for _, in := range d.model.InputData {
		...
		add(in.ID, "inputData", in.Name, t, "", false, false, false, false, false)
	}
	for _, b := range d.model.BKMs {
		add(b.ID, "businessKnowledgeModel", b.Name, canonicalType(b.VariableTypeRef), "", false, false, false, false, false)
	}
	for _, dec := range d.model.Decisions {
		...
		add(dec.ID, "decision", dec.Name, decisionOutputType(dec), varName, dec.DecisionTable != nil, dec.LiteralExpression != nil, dec.Context != nil, dec.Conditional != nil, dec.Logic() != nil)
	}
