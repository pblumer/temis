...
func (d *Definitions) Graph() Graph {
	...
	add := func(id, typ, name, dataType, varName string, hasTable, hasLiteral, hasContext, hasConditional, hasList, hasRelation, hasFilter, hasIterator, hasInvocation, hasLogic bool) {
		if id == "" {
			return
		}
		known[id] = true
		n := GraphNode{ID: id, Type: typ, Name: name, DataType: dataType, VarName: varName, HasTable: hasTable, HasLiteral: hasLiteral, HasContext: hasContext, HasConditional: hasConditional, HasList: hasList, HasRelation: hasRelation, HasFilter: hasFilter, HasIterator: hasIterator, HasInvocation: hasInvocation, HasLogic: hasLogic}
	...
	for _, dec := range d.model.Decisions {
		...
		add(dec.ID, "decision", dec.Name, decisionOutputType(dec), varName, dec.DecisionTable != nil, dec.LiteralExpression != nil, dec.Context != nil, dec.Conditional != nil, dec.List != nil, dec.Relation != nil, dec.Filter != nil, (dec.For != nil || dec.Quantified != nil), dec.Invocation != nil, dec.Logic() != nil)
	}
